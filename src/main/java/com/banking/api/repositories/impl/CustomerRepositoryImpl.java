...
public class CustomerRepositoryImpl implements CustomerRepository {
    ...
    private static final String SQL_FIND_BY_ID = "SELECT id, user_id, first_name, last_name, middle_initial, " +
            ...
            "FROM customers WHERE user_id = ? AND id = ?";
    ...
    private static final String SQL_UPDATE = "UPDATE customers SET first_name = ?, last_name = ?, middle_initial = ?, " +
            ...
            "country = ?, ssn = ?, created_date = ? WHERE user_id = ? AND id = ?";

    private static final String SQL_DELETE = "DELETE FROM customers WHERE user_id = ? AND id = ?";
    ...
    @Override
    public Customer findById(Long userId, Long id) throws BankResourceNotFoundException {
        try {
            return jdbcTemplate.queryForObject(SQL_FIND_BY_ID, customerRowMapper, userId, id);
        }catch (Exception e){
    ...
    @Override
    public void update(Long userId, Long id, Customer customer) throws BankBadRequestException {
        try {
            jdbcTemplate.update(SQL_UPDATE, customer.getFirstName(), customer.getLastName(), customer.getMiddleInitial(),
                    ...
                    customer.getCreatedDate(), userId, id);
        }catch (Exception e){
            throw new BankBadRequestException("Invalid request");
        }
    }
